...
* The published types use `NoInfer` on the configuration callbacks (`groupBy`, `itemFilter`, `createGroupHeaderItem`, `filter`) and on `selectionSnippet`, so consuming the types needs TypeScript >= 5.4. Only `items`, `value`, and `loadOptions` drive `Item` inference now: a pre-annotated callback const (or snippet) with a looser param type used to hijack `Item` and surface baffling weak-type errors on `items`/`loadOptions`; an incompatible callback now errors on the callback itself
...
* `loadOptions` is no longer re-run when the list opens or closes — it fires on typing (non-empty filter text), `loadOptionsDeps` changes, mount, and disabled toggles; closing the list never triggers a fetch. One exception on open: reopening onto results that do not reflect the current filter text re-fetches immediately — retained non-empty text (`clearFilterTextOnBlur={false}`) whose load was cancelled by the close or errored, or an empty input over query-narrowed results because the text was wiped at close (Escape). Mount and dependency loads now fire immediately instead of inheriting the typing debounce
...
* Enter and Escape pass through when the list is closed, so implicit form submission and dialog-close keep working; keys only stop propagation when the component actually handles them. Exception: in select-only mode (`searchable={false}`) Enter on a closed list opens it instead of passing through — the APG select-only combobox pattern specifies it, and a native-`<select>`-like control must not submit the form instead of opening
...
* `FilterConfig.value` is typed `Item | SelectItem | (Item | SelectItem)[] | null | undefined` instead of the dishonest `SelectItem`-only union — the entries are usually your items, and custom `filter` implementations no longer need a cast to read their own fields (implementations annotating the field need the widened type)
* `listStyle` is renamed `listStyles`, matching the `containerStyles`/`inputStyles` pluralization — the one singular in the trio was a recurring typo trap
* The unused `ScrollActionParams` interface is removed from the types module (it was never exported from the package)
...
* `justValue` hydration resolves against `string[]` items: a raw string item is its own id, but hydration compared by the `itemId` field (undefined on a string), so `<Select items={['one','two']} useJustValue justValue="one" />` rendered the placeholder forever while the equivalent `value="one"` worked
* Multiple-mode `justValue` hydration preserves the order of the seeded array instead of reordering entries to `items` order — the reordered array was then written back over the parent's bound `justValue` as a spurious "correction"
* A programmatic `bind:listOpen = false` cancels a load armed by typing, like `closeList()` already did — the debounced fetch used to fire a spinner and replace the items on the closed list
* An in-flight `loadOptions` response is discarded when the prop is removed mid-flight (it used to land items and fire `onloaded` on a Select with no loader), and the removal drops the now-unclearable `loading` flag
* Reopening the list under an emptied filter (Escape wiped the text) re-fetches the baseline results — the list used to show the previous query's narrowed results beneath an empty input with no re-fetch path
* Typing dismisses an armed arrow-key tag cursor when `loadOptions` is set — that branch skipped the reset, leaving the tag highlighted and announced while the user typed (Backspace itself was already guarded)
* Dev-only warning when items are missing their `itemId` field: such items all compare equal (`undefined === undefined`), so the first pick works and every later click reads "already selected" — a completely silent config error before this
* Select-only mode (`searchable={false}`) matches the APG select-only combobox pattern it implements: the input no longer advertises `aria-autocomplete="list"` (typing never refines a readonly input's list), and the default `ariaFocused` announcement drops its "type to refine list" instruction there (Enter now opening the closed list is under Breaking changes)
* The result count announced when the list opens (`ariaListOpen`) no longer includes presentational group headers — with `groupBy`, screen-reader users were told more results existed than the cursor could ever reach; selectable headers (`groupHeaderSelectable`) still count
* The visible empty/loading copy in the open list renders from the same `ariaEmpty`/`ariaLoading` props that drive the announcements, instead of hardcoded English — a localized consumer no longer shows one language while announcing another (`emptySnippet` still replaces the visual entirely)
* In single mode the input carries `aria-describedby` pointing at the rendered selection: the combobox's accessible value is only ever the (usually empty) filter text, so browse-mode/virtual-cursor users inspecting the control without focusing it had no way to read the current selection from the element itself
* Every activation target meets the 24px minimum (WCAG 2.2 2.5.8): the default theme's tag-remove buttons had shrunk to their 20px icon (now a 24px floor via `--multi-item-clear-min-width`), and the Tailwind theme's clear button (20px tall) and tag-remove buttons (20×20) are now 24px in both dimensions
...
* Pins for the select-only APG batch (Enter opens a closed list, no `aria-autocomplete`, the trimmed `ariaFocused` copy), the header-free announced count, the `aria-describedby` selection wiring, and the localized visible empty/loading copy; the browser suite measures the >=24px activation targets in both themes; `isGroupHeader` gained runtime coverage; and the two legacy justValue hydration tests assert the exact hydrated selection instead of mere existence
* Pins for the correctness wave: string-item `justValue` hydration (single and multiple, order preserved), the no-reordered-echo guarantee, programmatic-close load cancellation (new `ListOpenCloseTest` fixture), in-flight discard after loader removal, the emptied-filter reopen re-fetch, the tag-cursor reset while typing with `loadOptions`, and both arms of the missing-`itemId` dev warning
