...
* Tab only commits the highlighted option after expressed intent — moving the cursor (keys or mouse hover) or typing filter text. The cursor auto-parks on the first option when the list opens, so merely opening the control and tabbing away used to silently select that parked option; it now just closes the list (APG behavior). Single-press Tab-commit after navigating or filtering is unchanged
...
* The open listbox has an accessible name on the `id` + external `<label for>` path (ARIA 1.2): the label names only the input, not the floating list, so the listbox was unnamed unless `ariaLabel` was set. The associated label is now referenced via `aria-labelledby` (or its text is snapshotted into `aria-label` when it has no `id`); an explicit `ariaLabel` still wins
