...
* A slow `loadOptionsDeps` reload can no longer wipe a selection the user made from fresher results: when typing during the reload delivered post-deps-change options and the user picked one, the late reload response still validated (and cleared) that brand-new selection against its own older results — `oninput(null)` fired and the choice visibly vanished. A committed selection now retires the pending reload's validation authority whenever the displayed results it was picked from are newer than the reload; a pick made from genuinely stale (pre-deps-change) options is still validated as before
